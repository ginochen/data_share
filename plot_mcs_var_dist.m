function plot_mcs_var_dist(vname,season,dosave,donewplot)
...
%  diro = ['/Users/g/archive/matlab/figure/mcs_cluster/f09f09/' season];
  ...
  load('mcs_clusters_1.mat','nt4Cl','t','t4Cl','mcsnllave','mcsnll','nCl','lon','lat','ilatlim','latlim');
  load('mcs_cluster_parm.mat');
  load(['mcs_cluster_' vname '.mat']);
  evalc(sprintf(['vtmp =' vname ';']));
...
%  load([diri  '/mcs_cluster_var/mcs_cluster_precl.mat'])
  load(['/mcs_cluster_var/mcs_cluster_mcsillt4Cl.mat'])
...
%  cbarunits={'[m s^{-1}]','[mm hr^{-1}]','[mm hr^{-1}]'};
  ...
  caxislim={[0 10],[0 10],[0 10],[0 80],[0 15],[0 100],[-5 0],[0 3e-2],[0 2],[0 40]};
%  scale = [1,1,1,1,1,1/90];
  ...
  for ic=1:nCl
    ...
    for iitt=1:numel(mcsillt4Cl{ic})%nt4Cl(ic)+2
      for ill=1:size(mcsillt4Cl{ic}{iitt},1)
        ...
        if ~ismember(vname,{'fftke'})
%if isnan(varmap(l1,l2)); disp('nanana'); end
...
%    contourf(lon,lat(ilatlim(1):ilatlim(2)),varmap(:,ilatlim(1):ilatlim(2))',ncon,'linestyle','none');
%  end
  colormap(cmap(icmap(ivv(vname)),doflip(ivv(vname)),10)); hold on
  daspect([.50 .5 1])
  title([season])
...
% COLORBAR
  if docbar
    ...
    h.Label.Position = [caxislim{ivv(vname)}(2)+0.15*abs(diff(caxislim{ivv(vname)})) 0 0];
    set(gca,'Fontsize',44);
