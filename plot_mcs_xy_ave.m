function plot_mcs_xy_ave(vname,iic,ixshift,dosave)
...
% iic=259;vname={'LI','ps','du'};dosave=0;plot_mcs_xy_ave(vname,iic,dosave)
...
%  itvec = [3]; % rotate pvecmcs by 20deg
  itvec = 1:10;%[1:10]; %rotate pvecmcs by 20deg
%  itvec = [1:3]; %[1:10]
% itvec=[16]
end
...
disp(['current centroid lon lat=' num2str(parm.lon(mcsilltcentroids{ic}(itr,1))) ', ' num2str(parm.lat(mcsilltcentroids{ic}(itr,2)))]); 
if parm.lon(mcsilltcentroids{ic}(itr,1))<180
  disp(['current local time = UTC time + ' num2str(parm.lon(mcsilltcentroids{ic}(itr,1))/360*24) ' hours'])
else
  disp(['current local time = UTC time - ' num2str((parm.lon(mcsilltcentroids{ic}(itr,1))-180)/360*24) ' hours'])
end
...
%      for i=1:nj  
      for i=1:nj
        ...
        elseif docrosstype==2 % cut through mcs propag dir
          ix = ixp(ilonlat(i,1)); % lon indices along the vert cross section
          iy = iyp(ilonlat(i,2)); % lat ...
...
%          ilatcs(i) = iy(1);
 ...
 %       wcrmz(i,:,:,it) = mean(vo.w_crmi{ic}{itr(it)}(ix,iy,:,:),2);
        ...
        frac_preccz(i,:,it)   = mean(vo.frac_precc{ic}{itr(it)}(ix,iy,:),2);
        frac_preclz(i,:,it)   = mean(vo.frac_precl{ic}{itr(it)}(ix,iy,:),2);
        freq_preccz(i,:,it)   = mean(vo.freq_precc{ic}{itr(it)}(ix,iy,:),2);
        freq_preclz(i,:,it)   = mean(vo.freq_precl{ic}{itr(it)}(ix,iy,:),2);
...
%  if docross
    thetaeaz  = thetaez - mean(mean(thetaez,1),3); % thetaez is zonal-vert
%  end
...
%  sanePColor(lonc',latc',vo.prect{ic}{itr(it)}(ixp,iyp)'*3.6e6); hold on
%%{
  clevs = linspace(-20,0,5);
  contourf(lonc,latc,du',clevs,'linestyle','none'); 
%  contourf(lonc,latc,dv',[-20:2:0],'linestyle','none'); 
  %caxis([-20 20]); 
  caxis([clevs(1) clevs(end)]); 
  cm = colormap(hh(ip),cmap(6,1,numel(clevs)-1)); hold on; 
%%}
%  contour(lonc,latc,du',[-50:2:0],'b:'); 
%  contour(lonc,latc,du',[0:2:50],'-','color',[0.81 0.53 0.53]); 
  contour(lonc,latc,vo.prect{ic}{itr(it)}(ixp,iyp)'*3.6e6,[1:3:30],'r','linewidth',cs); hold on
%  contour(lonc,latc,thetaes'-350,[3 3],'k','linewidth',3);
%  contour(lonc,latc,thetaes'-350,[-0.5 -0.5],'m','linewidth',3);
%  contour(lonc,latc,thetaes'-mean(thetaes(:)),[-3 -3],'m--','linewidth',3); 
%  contour(lonc,latc,LI',[-1 -1],'b-','linewidth',3);  
%  contour(lonc,latc,thetaes'-mean(thetaes(:)),[1 1],'m','linewidth',3); 
%  contour(lonc,latc,B',[20:10:200],'c','linewidth',cs);
%  contour(lonc,latc,vo.freq_precc{ic}{itr(it)}(ixp,iyp)'); hold on
%  cm = colormap(hh(ip),cmap(6,0,16)); hold on; 
%  contour(lonc,latc,vo.freq_precl{ic}{itr(it)}(ixp,iyp)'); hold on
%  contour(lonc,latc,pc',[6:3:50],'r','linewidth',cs);  % 1[mm/hr] = 0.1[cm/hr] = 2.4[cm/day] => 5[mm/hr] = 12[cm/day]
...
% for iic = 259
% xlim([155 185]); ylim([-5 5])
  stormarrow_plan(lonc,latc,us,vs,uscl2,ixp,iyp,[0 0 0])
  %stormarrow_plan(lonc,latc,us1,vs1,uscl2,ixp,iyp,[0.5 0.5 0.5])
  crossline(-pvecmcs,lonc,latc);
docbar=1
  cbar(docbar,cm,[clevs(1) clevs(end)],clevs,vname{ivv},'m s^{-1}',iic)
  ip=ip+1;
%%%%%%%%%%%%%% LI %%%%%%%%%%%%%%%%% 
case 'LI'
  hh(ip) = subplot(nprow,ceil(npanel/nprow),ip);
  clevs = linspace(-7,-2,3);  % contour levels
  contourf(lonc,latc,LI',clevs,'linestyle','none');  caxis([clevs(1) clevs(end)]); 
  cm = colormap(hh(ip),cmap(31,1,numel(clevs)-1)); hold on; 
...
%  ncquiverref(lonc,latc,usf',vsf','m/s',10,'true',[1 1 1]);
  contour(lonc,latc,thetaes'-mean(thetaes(:)),[-3 -3],'m--','linewidth',3); 
  quiver(lonc,latc,usf'*0.1,vsf'*0.1,'color',[1 1 1],'autoscale','off');
%  quiver(lonc,latc,vo.u_cam{ic}{itr(it)}(ixp,iyp,ih)'*0.1,vo.v_cam{ic}{itr(it)}(ixp,iyp,ih)'*0.1,'autoscale','off')
  ...
  %stormarrow_plan(lonc,latc,us1,vs1,uscl2,ixp,iyp,[0.5 0.5 0.5])
  crossline(-pvecmcs,lonc,latc);
  cbar(docbar,cm,[clevs(1) clevs(end)],clevs,vname{ivv},'[K]',iic)
...
case 'spdtz'
  hh(ip) = subplot(nprow,ceil(npanel/nprow),ip);
  [x y] = domeshz(xsquall,parm); % x and z (not y)
  clev = [-100:10:100];
...
%  contour(x,y,mean(thetaaz,3)',[-3:1:-0.5],'m--','linewidth',cs);  % 1[mm/hr] = 0.1[cm/hr] = 2.4[cm/day] => 5[mm/hr] = 12[cm/day]
  contour(x,y,thetaeaz',[1:3:20],'m','linewidth',cs);  % 1[mm/hr] = 0.1[cm/hr] = 2.4[cm/day] => 5[mm/hr] = 12[cm/day], height average
  contour(x,y,thetaeaz',[-20:3:-1],'m--','linewidth',cs);  % 1[mm/hr] = 0.1[cm/hr] = 2.4[cm/day] => 5[mm/hr] = 12[cm/day]
...
%{
  contour(x,y,mean(qiz,3)'*1e3,[0.01:0.005:0.02],'color',[.7,.7,.7],'linewidth',cs);  % 1[mm/hr] = 0.1[cm/hr] = 2.4[cm/day] => 5[mm/hr] = 12[cm/day]
  contour(x,y,mean(qcz,3)'*1e3,[0.01:0.005:0.02],'color',[.5,.5,.5],'linewidth',cs);  % 1[mm/hr] = 0.1[cm/hr] = 2.4[cm/day] => 5[mm/hr] = 12[cm/day]
